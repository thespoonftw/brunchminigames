...
public class MiniBugs_OverlyComplicatedGrassSpawner : MonoBehaviour {
        ...
        for (int x = 0; x < xSteps; x++) {
            for (int y = 0; y < ySteps; y++) {
                ...
                if (Mathf.Pow(noise, NoiseExponent) >= NoiseThreshold) {
                    ...
                    g.transform.GetChild(0).GetComponent <SpriteRenderer>().color = GrassColorGradient.Evaluate(Random.Range(0f, 1f));
